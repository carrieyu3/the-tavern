...
Defines the constructors and private and public function implementation of the Character class
Tiziana Ligorio, Georgina Woo
*/
...
void Character::setName(const std::string& name)
{
    ...
    for(size_t i = 0; i<name.size();i++) //changed int i = 0 to be size_t i = 0 (make rebuild gives unsigned int error)
    {
...
*/
bool Character::operator==(const Character& second_character) const{
    if((getName() == second_character.getName() && getRace() == second_character.getRace() && getLevel() == second_character.getLevel()) && (isEnemy() == second_character.isEnemy() || !isEnemy() == !second_character.isEnemy())){
        return true; //under the condition that character1=character2 when they equal in name, race, level, AND enemy flag
    }
    return false;
}
...
*/
bool Character::operator!=(const Character& second_character) const{
    if(getName() != second_character.getName() || getRace() != second_character.getRace() || getLevel() != second_character.getLevel() || isEnemy() != second_character.isEnemy()){
        return true; //under the condition that character1!=character2 when either the name, race, level, OR enemy flag is not equal
    }
    return false;
}
...
void Character::display() const{
    ...
    std:: string an_enemy = isEnemy() == true ? "They are an enemy." : "They are not an enemy."; //ternary operator to convert bool{0,1} to string{"false", "true"}
